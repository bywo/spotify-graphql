...
function shouldStopIterate(
  ...
  switch (iterator._type) {
    case "cursor":
      if (!!iterator.total) {
        return (
          iterator.results.length >= maxResults ||
          iterator.results.length >= iterator.total ||
          !iterator.after
        );
      } else {
        return false;
...
export function paginatorFactory(
  ...
  return async function paginator(
    ...
    if (argsArray.length > 1 && isObject(argsArray[argsArray.length - 1])) {
      let queryStrings: { [k: string]: string } =
    ...
    let iterator: anyPagingIterator = newIterator;
    while (!shouldStopIterate(iterator, maxResults)) {
      iterator = await new Promise<anyPagingIterator>((resolve, reject) => {
...
export function paginatorFromVariables(
  strategy: PagingStrategyType,
  variables: IPaginatorFromVariablesOptions
) {
